...
export async function generateMetadata({ params }: { params: Promise<{ code: string }> }): Promise<Metadata> {
  ...
  return { title: problem ? `Best solutions for ${problem.statement.name}` : "No such problem" };
}
...
export default async function RankPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  return (
    <ProblemPage problem={problem} active="rank" title={`Best solutions for ${problem.statement.name}`}>
