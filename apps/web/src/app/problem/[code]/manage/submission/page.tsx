...
export async function generateMetadata({ params }: { params: Promise<{ code: string }> }): Promise<Metadata> {
  ...
  return { title: problem ? `Managing submissions for ${problem.name}` : "No such problem" };
}
...
export default async function ManageSubmissionsPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  return (
    <>
      <ProblemHeader problem={problem} active="manage" title={`Managing submissions for ${problem.name}`} />
      <div id="content-body">
        <ManageSubmissions
