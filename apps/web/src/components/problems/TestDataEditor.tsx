...
export function TestDataEditor({ code, initial }: { code: string; initial: Payload }) {
  ...
  const zipContents = useAction(api.problemData.zipContents);
  ...
  async function upload(file: File) {
    await withErrors(async () => {
      ...
      await updateData({ code, zipfile: file.name, zipfileStorageId: storageId });
      const contents = await zipContents({ code });
      setFiles(contents.files);
      setStatus(
        contents.error
          ? `The archive was stored, but its contents could not be read: ${contents.error}`
          : `${file.name} uploaded — ${contents.files.length} files.`,
      );
    });
  }
    ...
    <div className="grid gap-4">
      ...
      <Panel title="Problem data" bodyClassName="grid gap-4 p-3">
        <Field
          label="Data zip file"
          htmlFor="zipfile"
          hint={data.data?.zipfile ? `Currently ${data.data.zipfile}.` : "No archive has been uploaded."}
        >
        ...
        <Table aria-label="Test cases" dense scrollable={false}>
          <TableHeader>
            <TableRow>
              ...
              {visible.batchDependencies ? <TableHead className="w-32">Batch dependencies</TableHead> : null}
