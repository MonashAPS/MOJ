 ...
 */
const MARKS = new Map<string, LanguageMark>([
  ["Ada", { path: "", hex: "02F88C" }],
...
const FALLBACK_HEX = new Map<string, string>([
  ["ALGOL 68", "4B5563"],
  ["Awk", "1E6F5C"],
  ["Brain****", "2F2F2F"],
  ["COBOL", "005CA5"],
...
export function markFor(commonName: string): LanguageMark {
  const mark = MARKS.get(commonName);

  if (mark) return mark;

  return { path: "", hex: FALLBACK_HEX.get(commonName) ?? "64748B" };
}
