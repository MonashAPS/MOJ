...
function closeTo(actual: readonly number[], expected: readonly number[]): void {
  ...
  actual.forEach((value, i) => expect(value).toBeCloseTo(expected[i] as number, PRECISION));
}
...
describe('recalculate_ratings golden cases', () => {
  it('four newcomers, no ties', () => {
    const result = recalculateRatings([1, 2, 3, 4], [1500, 1500, 1500, 1500], [0, 0, 0, 0], [
      [],
      [],
      [],
      [],
    ...
    closeTo(result.mean, [
      2054.8117718081685, 1646.6551817318596, 1353.344881857663, 945.1882281918315,
    ]);
    closeTo(result.performance, [
      2182.367751638652, 1679.1300621513783, 1320.87001558157, 817.6322483613482,
    ]);
  });

  it('a tie in the middle scores both the same', () => {
    const result = recalculateRatings([1, 2.5, 2.5, 4], [1500, 1500, 1500, 1500], [0, 0, 0, 0], [
      [],
      [],
      [],
      [],
...
describe('rateContest', () => {
  ...
  it('skips virtual participations, excluded users and the unsubmitted', () => {
    const result = rateContest(
      [
        ...rows,
        { participationId: 'v', profileId: 'virtual', score: 100, cumtime: 0, tiebreaker: 0, submissionCount: 1, virtual: 1 },
        { participationId: 'x', profileId: 'excluded', score: 100, cumtime: 0, tiebreaker: 0, submissionCount: 1 },
        { participationId: 'z', profileId: 'nosubs', score: 0, cumtime: 0, tiebreaker: 0, submissionCount: 0 },
      ],
  ...
  it('applies the rating floor and ceiling to the previous rating', () => {
    const field = [
      { participationId: 'p1', profileId: 'low', score: 10, cumtime: 0, tiebreaker: 0, submissionCount: 1, lastRating: 900 },
      { participationId: 'p2', profileId: 'mid', score: 9, cumtime: 0, tiebreaker: 0, submissionCount: 1, lastRating: 1500 },
      { participationId: 'p3', profileId: 'high', score: 8, cumtime: 0, tiebreaker: 0, submissionCount: 1, lastRating: 2500 },
    ];
    expect(
      rateContest(field, { contest: { ratingFloor: 1000 } }).map((row) => row.profileId),
    ).toEqual(['mid', 'high']);
    expect(
      rateContest(field, { contest: { ratingCeiling: 2000 } }).map((row) => row.profileId),
    ).toEqual(['low', 'mid']);
  ...
  it('uses the prior history and the performance ceiling', () => {
    const result = rateContest(
      [
        { participationId: 'p1', profileId: 'a', score: 10, cumtime: 0, tiebreaker: 0, submissionCount: 1, lastMean: 1700, timesRated: 3 },
        { participationId: 'p2', profileId: 'b', score: 5, cumtime: 0, tiebreaker: 0, submissionCount: 1, lastMean: 1500, timesRated: 1 },
        { participationId: 'p3', profileId: 'c', score: 1, cumtime: 0, tiebreaker: 0, submissionCount: 1, lastMean: 1300, timesRated: 0 },
      ],
      {
...
describe('monotonicity', () => {
  ...
  it('never rewards a worse rank with a better performance or rating', () => {
    ...
    for (let trial = 0; trial < 40; trial++) {
      ...
      const { rating, performance } = recalculateRatings(
        ranking,
        oldMean,
        timesRanked,
        historical,
