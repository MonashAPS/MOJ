...
describe('classifyEvent', () => {
  it('masks everything at or after the freeze', () => {
    expect(classifyEvent(attempt('u', 'p', FREEZE_OFFSET, 'AC'), FREEZE_OFFSET)).toEqual([
      PENDING,
      true,
    ]);
    expect(classifyEvent(attempt('u', 'p', FREEZE_OFFSET + 1, 'WA'), FREEZE_OFFSET)).toEqual([
      PENDING,
      true,
    ]);
...
describe('applyFreeze', () => {
  ...
  it('reports who sees through the freeze', () => {
    ...
    expect(
      canSeeThroughFreeze(contest, createUser('staff', { permissions: ['see_private_contest'] })),
    ).toBe(true);
  });
});
