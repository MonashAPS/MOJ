...
function check(
  ...
  for (const [username, methods] of Object.entries(expectations)) {
    ...
    if (methods.is_editable_by !== undefined) {
      expect(blogPostIsEditableBy(post, viewer), `is_editable_by/${username}`).toBe(
...
describe('Comment.is_accessible_by', () => {
  it('defers to the page the comment hangs off', () => {
    ...
    expect(
      commentIsAccessibleBy({ type: 'problem', problem: publicProblem }, users.normal),
    ).toBe(true);
    expect(
      commentIsAccessibleBy({ type: 'problem', problem: privateProblem }, users.normal),
    ).toBe(false);
    ...
    expect(commentIsAccessibleBy({ type: 'contest', contest: visibleContest }, users.normal)).toBe(
      true,
    );
    expect(commentIsAccessibleBy({ type: 'contest', contest: hiddenContest }, users.normal)).toBe(
      false,
    );

    expect(
      commentIsAccessibleBy({ type: 'blog', post: visibleBlogPost }, users.normal, { now: NOW }),
    ).toBe(true);
    expect(
      commentIsAccessibleBy({ type: 'blog', post: basicBlogPost }, users.normal, { now: NOW }),
    ).toBe(false);
...
describe('Organization permissions', () => {
  ...
  it('recognises organization admins', () => {
    ...
    expect(organizationCanReviewAllRequests(OPEN_ORGANIZATION, users.staff_organization_admin)).toBe(
      true,
    );
  });
  ...
  it('shows a join request to its author, the org admins and the class admins', () => {
    ...
    expect(
      canViewOrganizationRequest(request, OPEN_ORGANIZATION, null, users.staff_organization_admin),
    ).toBe(true);
