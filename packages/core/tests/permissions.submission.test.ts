...
import { canSeeSubmissionDetail, resolveSubmissionSourceVisibility } from '../src/permissions.js';
import { computeContestSubmissionPoints } from '../src/judging.js';
import { isGraded, isLocked, longStatus, resultClass, shortStatus } from '../src/verdicts.js';
import type { ProblemRow, Viewer } from '../src/types.js';
import {
  DAY,
  NOW,
  commonUsers,
...
const fullAcProblem = createProblem('full_ac');
const lockedProblem = createProblem('locked');
const futureLockedProblem = createProblem('future_locked');
...
describe('SubmissionTestCase', () => {
  ...
  it('lets contest staff see in-contest submissions', () => {
    ...
    expect(canSeeSubmissionDetail(submission, users.normal, { problem, contest: viewSubs })).toBe(
      true,
    );
    ...
    expect(canSeeSubmissionDetail(submission, users.normal, { problem, contest: testerSees })).toBe(
      true,
    );
    ...
    expect(canSeeSubmissionDetail(submission, users.normal, { problem, contest: testerBlind })).toBe(
      false,
    );
  });
});
