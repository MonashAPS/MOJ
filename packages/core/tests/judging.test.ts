...
describe('computeGradingEnd', () => {
  ...
  it('zeroes a non-partial problem that fell short', () => {
    const cases = [
      testCase({ case: 1, points: 9, total: 10 }),
    ];
  ...
  it('tolerates null times and memories', () => {
    const result = computeGradingEnd(
      [testCase({ case: 1, time: null, memory: null })],
      problem,
    );
...
describe('computeContestSubmissionPoints', () => {
  it('scales the case points by the contest problem value', () => {
    expect(computeContestSubmissionPoints({ casePoints: 50, caseTotal: 100 }, { points: 10, partial: true })).toBe(5);
    expect(computeContestSubmissionPoints({ casePoints: 0, caseTotal: 0 }, { points: 10, partial: true })).toBe(0);
