...
export const USER_SAFE_TAGS: readonly string[] = [
  "h1", "h2", "h3", "h4", "h5", "h6",
  "b", "i", "strong", "em", "tt", "del", "kbd", "s", "abbr", "cite", "mark", "q", "samp",
  "small", "u", "var", "wbr", "dfn", "ruby", "rb", "rp", "rt", "rtc", "sub", "sup", "time",
  "data",
...
export const USER_SAFE_ATTRS: Readonly<Record<string, readonly string[]>> = {
  ...
  video: [
    "autoplay", "controls", "crossorigin", "height", "muted", "loop", "poster", "preload",
    "src", "width",
  ],
...
function mergeAttributeMaps(
  ...
  for (const map of maps) {
    for (const [tag, attrs] of Object.entries(map)) {
      const set = out[tag] ?? (out[tag] = new Set());
