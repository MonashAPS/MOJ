...
describe("problemData.initYaml", () => {
  ...
  test("refuses non-editors and manually managed problems", async () => {
    ...
    await expect(asUser(t, "nobody").query(api.problems.data.initYaml, { code: "aplusb" })).rejects.toThrow();

    await expect(asUser(t, "staff").query(api.problems.data.initYaml, { code: "managed" })).rejects.toThrow(
      /managed by hand/,
    );
  });
});
