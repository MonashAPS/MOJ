...
import { makeProblem, makeProfile } from "./fixtures.setup";
...
async function runAsRoot(t: TestConvex) {
  return await t.withIdentity({ subject: "user_root" }).mutation(api.admin.dedupe.dedupeNaturalKeys, {});
}

async function makeRoot(t: TestConvex): Promise<void> {
...
describe("admin/dedupe.dedupeNaturalKeys on the navigation bar", () => {
   ...
   */
  async function seedNavigation(t: TestConvex) {
...
describe("admin/dedupe.dedupeNaturalKeys on the problem taxonomy", () => {
   ...
   */
  async function seedTaxonomy(t: TestConvex) {
    return await t.run(async (ctx) => {
      ...
      const seededSide = await makeProblem(ctx, "aplusb", seededGroup, {
      ...
      const bothSides = await makeProblem(ctx, "helloworld", importedGroup, {
        typeIds: [seededType, importedType],
      });
      const untouched = await makeProblem(ctx, "fizzbuzz", importedGroup, { typeIds: [mathType] });
  ...
  test("finishes a repair too big for one transaction on the scheduler", async () => {
    ...
    const ids = await seedTaxonomy(t);
    await t.run(async (ctx) => {
      for (let i = 0; i < 520; i++) await makeProblem(ctx, `bulk${i}`, ids.seededGroup);
    });
  ...
  test("refuses anyone who is not a superuser", async () => {
    ...
    await t.run(async (ctx) => {
      await makeProfile(ctx, "clerk", { isStaff: true, permissions: ["judge.change_problemtype"] });
    });

    await expect(
      t.withIdentity({ subject: "user_clerk" }).mutation(api.admin.dedupe.dedupeNaturalKeys, {}),
