...
import { judgeRow } from "../lib/testing";
import { setupConvexTest, type TestConvex } from "./convexTest.setup";
import { makeGroup, makeProblem, makeProfile, makeSubmission } from "./fixtures.setup";
...
async function languages(t: TestConvex): Promise<Doc<"languages">[]> {
...
describe("importer.insertBatch upserts the reference tables", () => {
  ...
  test("a language id from the mapping resolves on a table imported after it", async () => {
    ...
    const problemId = await t.run(async (ctx) => {
      const groupId = await makeGroup(ctx, "imported");
      return await makeProblem(ctx, "dumped", groupId, { allowedLanguageIds: [languageId] });
    });
  ...
  test("a table with no natural key still just inserts", async () => {
    ...
    const ids = await t.run(async (ctx) => {
      ...
      return { problemId: await makeProblem(ctx, "aplusb", groupId) };
    });
...
describe("the judge handshake survives duplicate languages", () => {
  test("a deployment that already has two rows per key still handshakes", async () => {
    ...
    await t.run(async (ctx) => {
      ...
      await ctx.db.insert("judges", judgeRow("judge.example.com", JUDGE_KEY_HASH));
    });
...
async function seedDuplicates(t: TestConvex, submissionCount = 3): Promise<DuplicateFixture> {
  return await t.run(async (ctx) => {
    ...
    for (let i = 0; i < submissionCount; i++) {
      submissionIds.push(await makeSubmission(ctx, profileId, problemId, loser, { date: i }));
    }
    ...
    const judgeId = await ctx.db.insert("judges", judgeRow("judge.example.com", JUDGE_KEY_HASH));
...
describe("admin/languages.dedupeByKey", () => {
  test("merges the duplicates onto the imported row and repoints everything", async () => {
    ...
    const report = await t
      ...
      .mutation(api.admin.languages.dedupeByKey, {});
  ...
  test("is safe to run twice", async () => {
    const t = setupConvexTest();
    const ids = await seedDuplicates(t);
    const root = t.withIdentity({ subject: "user_root" });
  ...
  test("refuses anyone who is not a superuser", async () => {
    ...
    await t.run(async (ctx) => {
      await makeProfile(ctx, "clerk", { isStaff: true, permissions: ["judge.change_language"] });
    });

    await expect(
      t.withIdentity({ subject: "user_clerk" }).mutation(api.admin.languages.dedupeByKey, {}),
