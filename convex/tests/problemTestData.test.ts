...
import {
  ...
  sha256Hex,
  type T,
} from "../__tests__/fixtures.helpers";
...
async function fixture() {
  ...
  const setter = await makeProfile(t, {
    username: "setter",
    isStaff: true,
    permissions: ["judge.edit_own_problem", "judge.edit_all_problem"],
  });
  const outsider = await makeProfile(t, { username: "outsider" });

  await t.run(async (ctx) => {
    await ctx.db.insert("apiKeys", {
      keyHash: await sha256Hex(KEY),
      name: "problem repo",
      profileId: setter.profileId,
  ...
  return { t, languageId, problemId, setter, outsider };
}
...
describe("GET /judge/data", () => {
  ...
  it("streams the archive back with its hash and size", async () => {
    const { t } = await fixture();
    const judge = await makeJudge(t, { name: "local", key: "secret" });
    await publish(t, "aplusb", FIRST);

    const response = await t.fetch(dataUrl(judge, "aplusb"), { method: "GET" });
    expect(response.status).toBe(200);
  ...
  it("answers 404 when the site holds nothing and 409 on a stale hash", async () => {
    ...
    const nothing = await t.fetch(dataUrl(judge, "aplusb"), { method: "GET" });
    expect(nothing.status).toBe(404);
    expect(await nothing.json()).toEqual({ ok: false, error: "no data" });

    const unknownProblem = await t.fetch(dataUrl(judge, "nosuch"), { method: "GET" });
    ...
    const matched = await t.fetch(dataUrl(judge, "aplusb", current), { method: "GET" });
    expect(matched.status).toBe(200);

    const mismatched = await t.fetch(dataUrl(judge, "aplusb", stale), { method: "GET" });
  ...
  it("answers 403 to an unknown judge, a bad key and a blocked judge", async () => {
    const { t } = await fixture();
    const judge = await makeJudge(t, { name: "local", key: "secret" });
    const blocked = await makeJudge(t, { name: "blocked", key: "secret", isBlocked: true });
    await publish(t, "aplusb", FIRST);
    ...
    expect(
      (await t.fetch(dataUrl({ name: judge.name, key: "wrong" }, "aplusb"), { method: "GET" })).status,
    ).toBe(403);
...
describe("claiming with site-owned data", () => {
  async function queued(t: T, languageId: Id<"languages">, problemId: Id<"problems">) {
    ...
    return await makeSubmission(t, { profileId: author.profileId, problemId, languageId });
  }

  it("a judge that never reported the code claims when the site holds data", async () => {
    const { t, languageId, problemId } = await fixture();
    const judge = await makeJudge(t, { problemCodes: [], runtimeKeys: ["PY3"] });
    const submissionId = await queued(t, languageId, problemId);

    const before = await judgeClient(t, judge).claim();
    expect((await before.json()).submission).toBeNull();

    await publish(t, "aplusb", FIRST);

    const after = await judgeClient(t, judge).claim();
  ...
  it("a judge that reports the code still claims when the site holds nothing", async () => {
    ...
    const response = await judgeClient(t, judge).claim();
  ...
  it("the executor must still match", async () => {
    const { t, languageId, problemId } = await fixture();
    const judge = await makeJudge(t, { problemCodes: [], runtimeKeys: ["CPP17"] });
    await queued(t, languageId, problemId);
    await publish(t, "aplusb", FIRST);
...
describe("the test data editor", () => {
  it("publishes an uploaded zip through the same storage and reports its state", async () => {
    const { t, setter, problemId } = await fixture();
