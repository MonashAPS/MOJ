...
import { makeProfile } from "./fixtures.setup";

describe("rankings.users", () => {
  test("ties share a rank and the next distinct pair skips it", async () => {
    const t = setupConvexTest();
    await t.run(async (ctx) => {
      await makeProfile(ctx, "alice", { performancePoints: 300, problemCount: 10, points: 900 });
      await makeProfile(ctx, "bob", { performancePoints: 200, problemCount: 5, points: 500 });
      await makeProfile(ctx, "carol", { performancePoints: 200, problemCount: 5, points: 400 });
      await makeProfile(ctx, "dave", { performancePoints: 100, problemCount: 2, points: 100 });
    });
  ...
  test("equal points but different problem counts do not tie", async () => {
    const t = setupConvexTest();
    await t.run(async (ctx) => {
      await makeProfile(ctx, "one", { performancePoints: 200, problemCount: 7 });
      await makeProfile(ctx, "two", { performancePoints: 200, problemCount: 3 });
    });
  ...
  test("unlisted users never appear", async () => {
    const t = setupConvexTest();
    await t.run(async (ctx) => {
      await makeProfile(ctx, "listed", { performancePoints: 10 });
      await makeProfile(ctx, "hidden", { performancePoints: 5000, isUnlisted: true });
    });
  ...
  test("the page offset seeds the rank", async () => {
    ...
    await t.run(async (ctx) => {
      for (let i = 0; i < 105; i++) {
        await makeProfile(ctx, `u${String(i).padStart(3, "0")}`, {
  ...
  test("sorting by problem count keeps DMOJ's rank key", async () => {
    const t = setupConvexTest();
    await t.run(async (ctx) => {
      await makeProfile(ctx, "wide", { performancePoints: 50, problemCount: 40 });
      await makeProfile(ctx, "deep", { performancePoints: 900, problemCount: 3 });
    });
  ...
  test("an organisation filter lists only that organisation's members", async () => {
    const t = setupConvexTest();
    await t.run(async (ctx) => {
      const inside = await makeProfile(ctx, "member", { performancePoints: 10 });
      await makeProfile(ctx, "outsider", { performancePoints: 900 });
...
describe("rankings.find", () => {
  test("returns the rank and the page the user is on", async () => {
    ...
    await t.run(async (ctx) => {
      for (let i = 0; i < 150; i++) {
        await makeProfile(ctx, `p${String(i).padStart(3, "0")}`, {
          performancePoints: 1000 - i,
        });
      }
    });
  ...
  test("an unknown user is null and an unlisted user is flagged", async () => {
    const t = setupConvexTest();
    await t.run(async (ctx) => {
      await makeProfile(ctx, "ghost", { performancePoints: 10, isUnlisted: true });
    });
  ...
  test("ties are broken the same way the leaderboard breaks them", async () => {
    const t = setupConvexTest();
    await t.run(async (ctx) => {
      await makeProfile(ctx, "ahead", { performancePoints: 300 });
      await makeProfile(ctx, "tie-a", { performancePoints: 200 });
      await makeProfile(ctx, "tie-b", { performancePoints: 200 });
      await makeProfile(ctx, "behind", { performancePoints: 100 });
    });
  ...
  test("the page it reports is the page that shows the user", async () => {
    ...
    await t.run(async (ctx) => {
      for (let i = 0; i < 120; i++) {
        await makeProfile(ctx, `q${String(i).padStart(3, "0")}`, { performancePoints: 500 - i });
      }
    });
...
describe("rankings.top", () => {
  test("returns the highest performance points first", async () => {
    const t = setupConvexTest();
    await t.run(async (ctx) => {
      await makeProfile(ctx, "low", { performancePoints: 1 });
      await makeProfile(ctx, "high", { performancePoints: 99 });
      await makeProfile(ctx, "hidden", { performancePoints: 500, isUnlisted: true });
    });
