...
async function seedProblem(t: ReturnType<typeof setupConvexTest>) {
  return await t.run(async (ctx) => {
    const staffId = await makeProfile(ctx, "root", {
      isStaff: true,
      isSuperuser: true,
      permissions: ["judge.rejudge_submission", "judge.rejudge_submission_lot", "judge.edit_all_problem"],
    });
    const authorId = await makeProfile(ctx, "author");
    ...
    const submissionId = await makeSubmission(ctx, authorId, problemId, languageId, {
      result: "AC",
      points: 10,
      casePoints: 1,
      caseTotal: 1,
...
async function job(t: ReturnType<typeof setupConvexTest>, jobId: Id<"jobs">) {
...
describe("jobs.run dispatch", () => {
  ...
  test("admin.problems.rejudgeAll schedules a job that runs to completion", async () => {
    ...
    const { jobId } = await t
      ...
      .mutation(api.admin.problems.rejudgeAll, { code: "aplusb" });
    await t.finishAllScheduledFunctions(() => {});
...
describe("the cron targets", () => {
  test("the contest-mode sweep clears a participation whose contest is gone", async () => {
    ...
    await t.run(async (ctx) => {
      const profileId = await makeProfile(ctx, "ghost");
      const contestId = await makeContest(ctx, "over");
