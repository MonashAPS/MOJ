...
async function fixture(problemOptions: Parameters<typeof makeProblem>[1] = {}) {
  const t = setupTest();
  const languageId = await makeLanguage(t, "PY3");
  const otherLanguageId = await makeLanguage(t, "CPP17");
...
describe("submissions.submit", () => {
  ...
  it("refuses a banned user", async () => {
    ...
    await makeProblem(t, {
      code: "aplusb",
      allowedLanguageIds: [languageId],
      bannedProfileIds: [author.profileId],
    });
    await expect(
      t.withIdentity({ subject: author.userId }).mutation(api.submissions.submit, {
        problemCode: "aplusb",
        languageKey: "PY3",
        source: "print(1)",
      }),
  ...
  it("refuses a problem the user cannot see", async () => {
    const { t, author } = await fixture({ isPublic: false });
    await expect(
      t.withIdentity({ subject: author.userId }).mutation(api.submissions.submit, {
        problemCode: "aplusb",
        languageKey: "PY3",
  ...
  it("stops at DMOJ_SUBMISSION_LIMIT submissions in flight", async () => {
    ...
    await makeSubmission(t, { profileId: author.profileId, problemId, languageId, status: "QU" });
    await makeSubmission(t, { profileId: author.profileId, problemId, languageId, status: "G" });
  ...
  it("does not count rejudged submissions towards the in-flight limit", async () => {
    ...
    const result = await t.withIdentity({ subject: author.userId }).mutation(api.submissions.submit, {
      problemCode: "aplusb",
      languageKey: "PY3",
      source: "print(1)",
    });
  ...
  it("lets judge.spam_submission past the limits", async () => {
    ...
    const author = await makeProfile(t, { permissions: ["judge.spam_submission"] });
    await makeSubmission(t, { profileId: author.profileId, problemId, languageId, status: "QU" });
    await makeSubmission(t, { profileId: author.profileId, problemId, languageId, status: "G" });
  ...
  it("refuses a judge pin from someone who cannot edit the problem", async () => {
    const { t, author } = await fixture();
    const judge = await makeJudge(t, { name: "local" });
    await expect(
      t.withIdentity({ subject: author.userId }).mutation(api.submissions.submit, {
        problemCode: "aplusb",
        languageKey: "PY3",
  ...
  it("pins to a judge for a problem editor", async () => {
    ...
    await makeProblem(t, {
      code: "aplusb",
      allowedLanguageIds: [languageId],
      authorProfileIds: [editor.profileId],
    });
    const judge = await makeJudge(t, { name: "local" });

    const { submissionId } = await t
      .withIdentity({ subject: editor.userId })
      .mutation(api.submissions.submit, {
        problemCode: "aplusb",
        languageKey: "PY3",
        source: "print(1)",
        judgePin: judge.name,
      });
    expect((await t.run(async (ctx) => ctx.db.get(submissionId)))?.judgePin).toBe(judge.judgeId);
  });
});
...
describe("submissions.abort", () => {
  it("aborts a queued submission immediately", async () => {
    ...
    const result = await t
      ...
      .mutation(api.submissions.abort, { submissionId });
  ...
  it("leaves a finished submission alone", async () => {
    ...
    await t.withIdentity({ subject: author.userId }).mutation(api.submissions.abort, { submissionId });
  ...
  it("refuses someone else's submission without abort_any_submission", async () => {
    ...
    await expect(
      t.withIdentity({ subject: stranger.userId }).mutation(api.submissions.abort, { submissionId }),
    ).rejects.toThrow(/may not abort/);

    const staff = await makeProfile(t, { permissions: ["judge.abort_any_submission"] });
    await t.withIdentity({ subject: staff.userId }).mutation(api.submissions.abort, { submissionId });
  ...
  it("refuses a rejudged submission to its own author", async () => {
    ...
    await expect(
      t.withIdentity({ subject: author.userId }).mutation(api.submissions.abort, { submissionId }),
    ).rejects.toThrow(/may not abort/);
  });
});
...
describe("submissions.rejudge", () => {
  it("resets everything judgeapi.judge_submission resets", async () => {
    const { t, languageId, problemId, author } = await fixture();
    const staff = await makeProfile(t, { permissions: ["judge.rejudge_submission"] });
    ...
    await t.withIdentity({ subject: staff.userId }).mutation(api.submissions.rejudge, { submissionId });
  ...
  it("refuses a submission a judge is already grading", async () => {
    ...
    await expect(
      t.withIdentity({ subject: staff.userId }).mutation(api.submissions.rejudge, { submissionId }),
    ).rejects.toThrow(/already being judged/);
  });
  ...
  it("refuses without the permission, and refuses a locked submission", async () => {
    ...
    await expect(
      t.withIdentity({ subject: author.userId }).mutation(api.submissions.rejudge, { submissionId }),
    ).rejects.toThrow(/judge.rejudge_submission/);
    await expect(
      t.withIdentity({ subject: staff.userId }).mutation(api.submissions.rejudge, { submissionId }),
