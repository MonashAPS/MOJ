...
async function fixture() {
  ...
  const author = await makeProfile(t, { username: "author" });
...
describe("listContext filter options", () => {
  it("never offers SC, and offers IE only to staff", async () => {
    ...
    const member = await makeProfile(t, { username: "member" });
    const staff = await makeProfile(t, { username: "staff", isStaff: true });

    const asMember = await t
      .withIdentity({ subject: member.userId })
      .query(api.pages.submissions.listContext, {});
...
describe("listContext subjects", () => {
  ...
  it("marks the viewer's own list as theirs", async () => {
    const { t } = await fixture();
    const me = await makeProfile(t, { username: "me" });

    const own = await t
      .withIdentity({ subject: me.userId })
      .query(api.pages.submissions.listContext, { username: "me" });
    expect(own.user?.isSelf).toBe(true);

    const other = await t
      .withIdentity({ subject: me.userId })
      .query(api.pages.submissions.listContext, { username: "author" });
    expect(other.user?.isSelf).toBe(false);
  });

  it("refuses a problem the viewer cannot access", async () => {
    const { t } = await fixture();
    await makeProblem(t, { code: "secret", isPublic: false });
...
describe("statusExtras", () => {
  ...
  it("names the judge only for someone who can edit the problem", async () => {
    ...
    const judge = await makeJudge(t, { name: "judge-a" });
    ...
    await t.run(async (ctx) => ctx.db.patch(submissionId, { judgedOnJudgeId: judge.judgeId }));

    const asOwner = await t
      .withIdentity({ subject: owner.userId })
      .query(api.pages.submissions.statusExtras, { submissionId: 7 });
  ...
  it("prefers the language's time limit and reports the slowest case", async () => {
    ...
    const extras = await t
      .withIdentity({ subject: owner.userId })
      .query(api.pages.submissions.statusExtras, { submissionId: 11 });
  ...
  it("lets the author abort their own submission but not a rejudge", async () => {
    const { t, languageId, problemId } = await fixture();
    const owner = await makeProfile(t, { username: "owner" });
    const mine = await makeSubmission(t, {
      profileId: owner.profileId,
      problemId,
      languageId,
      status: "G",
    ...
    const own = await t
      .withIdentity({ subject: owner.userId })
      .query(api.pages.submissions.statusExtras, { submissionId: 21 });
    ...
    const rejudged = await t
      .withIdentity({ subject: owner.userId })
      .query(api.pages.submissions.statusExtras, { submissionId: 22 });
...
describe("sourceView", () => {
  it("withholds the source from someone who may not read it", async () => {
    ...
    const asOwner = await t
      .withIdentity({ subject: owner.userId })
      .query(api.pages.submissions.sourceView, { submissionId: 31 });
