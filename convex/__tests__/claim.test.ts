...
async function fixture() {
  ...
  const other = await makeProblem(t, { code: "other", allowedLanguageIds: [languageId] });
...
describe("judge authentication", () => {
  it("rejects an unknown judge, a bad key and a blocked judge", async () => {
    ...
    await makeJudge(t, { name: "local", key: "secret" });
    await makeJudge(t, { name: "blocked", key: "secret", isBlocked: true });

    expect((await judgeClient(t, { judgeId: "" as never, name: "nope", key: "x" }).claim()).status).toBe(403);
    expect((await judgeClient(t, { judgeId: "" as never, name: "local", key: "wrong" }).claim()).status).toBe(
      403,
    );
    expect(
      (await judgeClient(t, { judgeId: "" as never, name: "blocked", key: "secret" }).claim()).status,
    ).toBe(403);
    expect(
      (await judgeClient(t, { judgeId: "" as never, name: "local", key: "secret" }).claim()).status,
    ).toBe(200);
  });

  it("stores the problem list and the runtimes from the handshake", async () => {
    const { t } = await fixture();
    const judge = await makeJudge(t, { problemCodes: [], runtimeKeys: [] });
    ...
    expect(await response.json()).toEqual({ ok: true, judgeId: judge.judgeId });

    const stored = await t.run(async (ctx) => ctx.db.get(judge.judgeId));
    ...
    const runtimes = await t.run(async (ctx) =>
      ctx.db
        ...
        .withIndex("by_judge", (q) => q.eq("judgeId", judge.judgeId))
        .collect(),
    );
  ...
  it("takes load and a new problem list on the heartbeat", async () => {
    const { t } = await fixture();
    const judge = await makeJudge(t, { problemCodes: ["aplusb"] });
    const client = judgeClient(t, judge);
...
describe("claimNext", () => {
  it("takes the oldest submission of the lowest priority first", async () => {
    ...
    const judge = await makeJudge(t, { problemCodes: ["aplusb"], runtimeKeys: ["PY3"] });
    const client = judgeClient(t, judge);
  ...
  it("only claims problems and languages the judge has", async () => {
    ...
    const judge = await makeJudge(t, { problemCodes: ["aplusb"], runtimeKeys: ["PY3"] });
    const body = (await (await judgeClient(t, judge).claim()).json()) as {
  ...
  it("honours judgePin", async () => {
    const { t, languageId, problemId, author } = await fixture();
    const pinned = await makeJudge(t, { name: "pinned" });
    const other = await makeJudge(t, { name: "other" });
    await makeSubmission(t, {
      profileId: author.profileId,
      problemId,
      languageId,
      legacyId: 7,
      judgePin: pinned.judgeId,
    });

    const otherBody = (await (await judgeClient(t, other).claim()).json()) as { submission: unknown };
  ...
  it("only lets the minimum online tier claim", async () => {
    const { t, languageId, problemId, author } = await fixture();
    await makeJudge(t, { name: "fast", tier: 0 });
    const slow = await makeJudge(t, { name: "slow", tier: 5 });
    await makeSubmission(t, { profileId: author.profileId, problemId, languageId, legacyId: 4 });
  ...
  it("promotes the higher tier once the lower one goes offline", async () => {
    const { t, languageId, problemId, author } = await fixture();
    const fast = await makeJudge(t, { name: "fast", tier: 0 });
    const slow = await makeJudge(t, { name: "slow", tier: 5 });
    await makeSubmission(t, { profileId: author.profileId, problemId, languageId, legacyId: 5 });

    await t.run(async (ctx) => ctx.db.patch(fast.judgeId, { online: false }));
  ...
  it("reserves the last free judge of a tier for non-rejudge work", async () => {
    const { t, languageId, problemId, author } = await fixture();
    const busy = await makeJudge(t, { name: "busy" });
    const free = await makeJudge(t, { name: "free" });

    const held = await makeSubmission(t, {
      profileId: author.profileId,
      problemId,
      languageId,
      status: "G",
    ...
    await t.run(async (ctx) => ctx.db.patch(busy.judgeId, { currentSubmissionId: held }));
    ...
    const reserved = (await (await judgeClient(t, free).claim()).json()) as { submission: unknown };
  ...
  it("claims a rejudge when it is the only judge", async () => {
    const { t, languageId, problemId, author } = await fixture();
    const only = await makeJudge(t, { name: "only" });
    await makeSubmission(t, {
      profileId: author.profileId,
      problemId,
      languageId,
  ...
  it("marks the submission processing and the judge busy", async () => {
    ...
    expect(stored?.claimedByJudgeId).toBe(judge.judgeId);
    expect(stored?.judgedOnJudgeId).toBe(judge.judgeId);

    const judgeRow = await t.run(async (ctx) => ctx.db.get(judge.judgeId));
    expect(judgeRow?.currentSubmissionId).toBe(submissionId);

    // A busy judge claims nothing more.
    const again = (await (await judgeClient(t, judge).claim()).json()) as { submission: unknown };
  ...
  it("resolves a per-language limit override", async () => {
    ...
    const judge = await makeJudge(t);
    await makeSubmission(t, { profileId: author.profileId, problemId, languageId, legacyId: 50 });
