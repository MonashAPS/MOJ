...
async function fixture(problemOptions: Parameters<typeof makeProblem>[1] = {}) {
  ...
  return { t, languageId, problemId, author, judge, client };
}
...
describe("the grading sequence", () => {
  ...
  it("aborts a submission the judge already holds", async () => {
    ...
    await t.withIdentity({ subject: author.userId }).mutation(
  ...
  it("records a compile error and frees the judge", async () => {
    const { t, languageId, problemId, author, judge, client } = await fixture();
    const submissionId = await makeSubmission(t, {
      profileId: author.profileId,
      problemId,
      languageId,
